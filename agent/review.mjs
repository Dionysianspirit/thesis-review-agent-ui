...
export const FIRST_PASS_PROMPT = [
  ...
  "先 list_outline。禁止为了省事一次列出全文。导航次数有限，每次导航结果都带 nav_left；额度不足时再取舍小节，不要重复尝试已失败的操作。",
  "覆盖底线：摘要（或绪论/引言）和结论必须实际 read_section；每一个一级标题（1、2、3…）至少读一处有正文的小节，不能只读章节标题、不能整章跳过。图表题注不是一章。然后再按疑点补读方法、实验、结果等小节。",
  "章节覆盖是接触记录，不是通读全文的证明。coverage_status 的 read=实际读到段落，probed=仅检索命中，unread=这次没打开过。提交前若 nav_left>0 且仍有未读到的正文小节，继续读，不要提交。",
...
function allTools(call) {
  return [
    ...
    makeTool(call, "coverage_status", "章节覆盖", "查看每个大纲标题的接触状态：read=实际读到段落，probed=仅检索命中，unread=未打开过；并返回剩余导航额度。额度未用尽时优先补齐未读到的正文小节，不要整章跳过。", Type.Object({})),
